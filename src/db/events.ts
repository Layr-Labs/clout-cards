 ...
 * @param handler - Function that processes the RPC call and mutates state
 *
...
export async function withEvent<T>(
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    // Create the event (this will use the transaction context)
    await createEvent(kind, payloadJson, player, nonce);
